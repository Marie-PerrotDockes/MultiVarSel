...
```
The $p-value$ is really small hence we reject the hypothesis that each row of the residual matrix is a white noise which is an expected result since each row of $E$ has been generated via an AR(1) process.
...
```{r}
result <- whitening_choice(E_hat, c("AR1", "ARMA", "nonparam"), pAR = 1, qMA = 1)
result
```
...
```{r}
Frequencies <- variable_selection(Y = Y, X = X, nb_replis = 50, typeDep =  "AR1")
```

In the previous command line, \texttt{nb\_replis} corresponds to the number of replications which is used in the stability selection. Here it is equal to 50 but in practice we recommend to take it equal to 1000. The following plot displays the frequencies at which each coefficient of $B$ is considered as being non null. 

```{r, echo = FALSE, fig.keep = TRUE}
p <- ggplot(
    data = Frequencies[Frequencies$Frequencies >= 0.95, ],
...
```{r}
data("copals_camera")
Y <- data.frame(Y) %>% select(1:200) %>% scale()
```
